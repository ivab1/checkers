...
void drawEasyText(float x, float y, const char* text, float scale) {
    ...
    glVertexPointer(2, GL_FLOAT, 16, buffer); // Определяем указатель на вершины
    glDrawArrays(GL_QUADS, 0, quads * 4); // Рисуем массив квадратов
...
void drawGameOverMessage(const char* gameOverMessage, const char* winnerMessage) {
    ...
    drawEasyText(165, 500, "Press esc or R to return to menu", 2);
}